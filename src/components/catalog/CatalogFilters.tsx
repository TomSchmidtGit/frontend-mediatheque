...
const CatalogFilters: React.FC<CatalogFiltersProps> = ({
  ...
  const mediaTypes = [
    ...
    { value: 'movie', label: 'Films', icon: FilmIcon, color: 'text-purple-600' },
    { value: 'music', label: 'Musique', icon: MusicalNoteIcon, color: 'text-green-600' },
  ];
        ...
        <div className="grid grid-cols-3 gap-2">
          {mediaTypes.map((type) => {
            ...
            return (
              <button
                key={type.value}
                onClick={() => updateFilter('type', isSelected ? '' : type.value)}
                disabled={loading}
                className={cn(
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {filters.category && (
              <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                Catégorie: {categories.find(c => c._id === filters.category)?.name}
                <button
