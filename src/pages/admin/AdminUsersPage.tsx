...
const AdminUsersPage: React.FC = () => {
  const metaTags = generateMetaTags('adminUsers');
  ...
  const toggleStatusMutation = useMutation({
    ...
    onSuccess: (_, { userId }) => {
      ...
      toast.success(
        `Utilisateur ${user?.actif ? 'désactivé' : 'activé'} avec succès`
      );
    },
    onError: (error: any) => {
      const message =
              ...
              <div className='flex items-center justify-between'>
                <h2 className='text-lg font-semibold text-gray-900'>
                  {isLoading ? (
                    ...
                    <>
                      {usersData?.totalItems || 0} utilisateur
                      {(usersData?.totalItems || 0) > 1 ? 's' : ''} trouvé
                      {(usersData?.totalItems || 0) > 1 ? 's' : ''}
                    </>
                  )}
                </h2>

                <div className='text-sm text-gray-500'>
                  Page {usersData?.currentPage || 1} sur{' '}
                          ...
                          <div className='flex items-start space-x-3'>
                            ...
                            <div className='flex-1 min-w-0'>
                              ...
                              <div className='text-xs text-gray-500 mt-1'>
                                Inscrit le {formatDate.short(user.createdAt)} •{' '}
                                {Array.isArray(user.favorites)
                                  ? user.favorites.length
                                  : 0}{' '}
                                favoris
            ...
            <div className='flex items-start'>
              ...
              <div>
                ...
                <ul className='text-sm text-blue-800 space-y-1'>
                  <li>
                    • Vérifiez régulièrement les comptes inactifs et contactez
                    les utilisateurs
                  </li>
                  <li>
                    • Accordez les rôles d'employé avec prudence pour maintenir
                    la sécurité
                  </li>
                  <li>
                    • Surveillez les utilisateurs avec beaucoup de favoris pour
                    des recommandations
                  </li>
                  <li>
                    • Utilisez la recherche pour retrouver rapidement un
          ...
          onConfirm={() => {
            if (confirmUser) {
              toggleStatusMutation.mutate({
                ...
                currentStatus: confirmUser.actif,
              });
            }
          }}
        />
