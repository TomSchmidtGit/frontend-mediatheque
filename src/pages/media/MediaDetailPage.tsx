...
const MediaDetailPage: React.FC = () => {
  ...
  const {
    ...
    queryKey: ['media', id],
    queryFn: () => id ? mediaService.getMediaById(id) : Promise.reject('No ID'),
    enabled: !!id,
  });
  ...
  const addReviewMutation = useMutation({
    mutationFn: ({ mediaId, rating, comment }: { mediaId: string, rating: number, comment?: string }) =>
  ...
  const mockMedia: Media = {
    ...
    year: 1954,
    available: true,
    description: 'Dans un trou vivait un hobbit. Non point un trou déplaisant, sale et humide, rempli de bouts de vers et d\'une odeur suintante, non plus qu\'un trou sec, nu, sablonneux, sans rien pour s\'asseoir ni sur quoi manger : c\'était un trou de hobbit, ce qui implique le confort.',
    ...
    tags: [
      { _id: '1', name: 'aventure', slug: 'aventure', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
      { _id: '2', name: 'épique', slug: 'epique', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }
    ],
    reviews: [
      {
        _id: '1',
        user: 'user1',
        rating: 5,
        comment: 'Un chef-d\'œuvre absolu ! Tolkien a créé un univers d\'une richesse incroyable.',
      ...
      },
      {
        ...
        user: 'user2',
        rating: 4,
        comment: 'Excellent livre, même si parfois un peu long dans les descriptions.',
  ...
  const userHasReviewed = displayMedia.reviews.some(review => review.user === user?._id);
    ...
    return (
      <div className="page-container py-16">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-4">Média introuvable</h1>
          <p className="text-gray-600 mb-4">Le média que vous recherchez n'existe pas ou a été supprimé.</p>
  ...
  const renderStars = (rating: number, interactive = false, onRatingChange?: (rating: number) => void) => {
    return Array.from({ length: 5 }, (_, index) => {
      const StarComponent = index < Math.floor(rating) ? StarIconSolid : StarIcon;
      return (
        <StarComponent
          key={index}
          className={cn(
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <span className={cn(
                    ...
                    {getTypeIcon(displayMedia.type)}
                    <span className="ml-1">{formatters.mediaType(displayMedia.type)}</span>
                  </span>
                  ...
                  {!isAuthenticated && (
                    <p className="text-sm text-gray-400 mt-2">
                      <Link to="/login" className="text-primary-600 hover:text-primary-700">
                        Connectez-vous
                      </Link> pour laisser le premier avis
                    </p>
                  )}
                </div>
              )}
