...
class AdminBorrowService {
   ...
   */
  async getBorrows(filters: BorrowFilters = {}): Promise<PaginatedResponse<Borrow>> {
    ...
    const response = await api.get<PaginatedResponse<Borrow>>(`/borrow?${params}`);
  ...
  async returnBorrow(borrowId: string): Promise<{ message: string }> {
    const response = await api.put<{ message: string }>(`/borrow/${borrowId}/return`);
    return response.data;
  }
   ...
   */
  async getUserBorrows(userId: string, page: number = 1, limit: number = 10): Promise<PaginatedResponse<Borrow>> {
    const params = new URLSearchParams();
    params.append('page', page.toString());
    params.append('limit', limit.toString());
    
    const response = await api.get<PaginatedResponse<Borrow>>(`/borrow/user/${userId}?${params}`);
    return response.data;
  }
  ...
  async sendOverdueReminder(borrowId: string): Promise<{ message: string }> {
    const response = await api.post<{ message: string }>(`/borrow/${borrowId}/reminder`);
    return response.data;
  }
   ...
   */
  async getOverdueBorrows(page: number = 1, limit: number = 20): Promise<PaginatedResponse<Borrow>> {
    const params = new URLSearchParams();
    params.append('page', page.toString());
    params.append('limit', limit.toString());
  ...
  async cancelBorrow(borrowId: string): Promise<{ message: string }> {
    const response = await api.delete<{ message: string }>(`/borrow/${borrowId}`);
    return response.data;
  }
}
